...
OBSERVE = r"""(()=>{const vw=innerWidth,vh=innerHeight,touch=vw<820;const out=[];
 ...
 if(touch) for(const e of document.querySelectorAll('a[href],button')){ if(!vis(e))continue; const r=e.getBoundingClientRect();
   /* a STRETCHED link (::after with position:absolute; inset:0 over a positioned card) is tapped on its card, not its text box — measure the pseudo-element's box when it has one (2026-09-15: six false TAP-TARGETs on whole-card product cards) */
   const ps=getComputedStyle(e,'::after'); let tw=r.width, th=r.height; if(ps.content!=='none'&&ps.position==='absolute'){const pw=parseFloat(ps.width),ph=parseFloat(ps.height); if(pw>tw)tw=pw; if(ph>th)th=ph;}
   if((tw<44||th<44)&&!e.closest('p,li,td,dd,figcaption')&&hasText(e)) out.push(['TAP-TARGET',`${Math.round(tw)}×${Math.round(th)} “${e.textContent.trim().slice(0,28)}”`]); }
 ...
 {const ys=[];for(const e of document.querySelectorAll('main *')){if(!vis(e)||e.closest('table,.rcpt-rows,.rcpt-r,.rcpt-r-tight,.rcpt-box,.fr,.td-block,.ia-toc,.lh-log,.lint-findings,.lint-stats,.rule-list,pre,code,button,.cta,[class*="-chrome"],.recon,.recon-badge,.recon-caption,.vg,.lh-inst,.dsg-card,.hero-demo,.hd-card,.pd,.vid-frame,.card-wire,.artalt,.thesis,.card-gold,.fig-paper,.bp,.boarding,[class^="rx"],[class^="pl"][class*="-"]')   /* 2026-09-15: the same exemptions as rule-density-check — an instrument or reconstruction draws its own lines; the sweep counted a VC dossier's rows as a grid of rules */)continue;const c=getComputedStyle(e);const r=e.getBoundingClientRect();if(r.width<vw*0.4)continue;if(parseFloat(c.borderLeftWidth)>=1&&parseFloat(c.borderRightWidth)>=1)continue;
